  ...
  return (
    <div className="max-w-5xl mx-auto bg-white rounded-[2rem] shadow-xl border border-slate-100 overflow-hidden">
        ...
        <div className="p-8 border-b border-slate-100 flex justify-between items-center bg-slate-50">
            ...
            {onClose && (
                <button onClick={onClose} className="p-2 bg-white rounded-full hover:bg-red-50 text-slate-400 hover:text-red-500 transition-colors">
                    <X className="w-6 h-6"/>
                </button>
            )}
        </div>
      ...
      <form onSubmit={handleSubmit}>
        ...
        <div className="p-8 border-b border-slate-100">
            <h3 className="text-sm font-black text-slate-800 uppercase mb-6 flex items-center gap-2"><User className="text-[#FFC72C]"/> Osnovni Podaci</h3>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div className="space-y-1">
                    <label className="text-[10px] font-bold text-slate-400 uppercase">Ime i Prezime</label>
                    <input required value={formData.name} onChange={e => setFormData({...formData, name: e.target.value})} className="w-full border-2 border-slate-100 p-3 rounded-xl font-bold text-slate-700 outline-none focus:border-[#1a3826]"/>
                </div>
                <div className="space-y-1">
                    <label className="text-[10px] font-bold text-slate-400 uppercase">Email</label>
                    <input required type="email" value={formData.email} onChange={e => setFormData({...formData, email: e.target.value})} className="w-full border-2 border-slate-100 p-3 rounded-xl font-bold text-slate-700 outline-none focus:border-[#1a3826]"/>
                </div>
                <div className="space-y-1">
                    <label className="text-[10px] font-bold text-slate-400 uppercase">Lozinka {initialData && "(Ostaviti prazno ako se ne mijenja)"}</label>
                    <input type="password" value={formData.password} onChange={e => setFormData({...formData, password: e.target.value})} className="w-full border-2 border-slate-100 p-3 rounded-xl font-bold text-slate-700 outline-none focus:border-[#1a3826]"/>
                </div>
            </div>
        </div>
        ...
        <div className="p-8 border-b border-slate-100 bg-slate-50/50">
            <h3 className="text-sm font-black text-slate-800 uppercase mb-4 flex gap-2"><Store className="text-[#FFC72C]"/> Dodijeli Restorane</h3>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                {restaurants.map(r => {
                    const isSelected = selectedRestaurants.includes(r.id);
                    return (
                        <div key={r.id} onClick={() => toggleRestaurant(r.id)} 
                             className={`cursor-pointer p-4 rounded-xl border-2 transition-all flex items-center justify-between group ${isSelected ? 'border-[#1a3826] bg-[#1a3826] text-white shadow-lg transform scale-[1.02]' : 'border-slate-200 bg-white hover:border-slate-300'}`}>
        ...
        {/* 3. ULOGA & BENEFITI */}
        <div className="p-8 border-b border-slate-100">
            <h3 className="text-sm font-black text-slate-800 uppercase mb-6 flex gap-2"><Shield className="text-[#FFC72C]"/> Pozicija & Godišnji</h3>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="space-y-4">
                    <div>
                        <label className="text-[10px] font-bold text-slate-400 uppercase">Uloga</label>
                        <select value={formData.role} onChange={e => setFormData({...formData, role: e.target.value})} className="w-full border-2 border-slate-100 p-3 rounded-xl font-bold text-slate-700 outline-none focus:border-[#1a3826]">
                    ...
                    <div>
                        ...
                        <select value={formData.department} onChange={e => setFormData({...formData, department: e.target.value})} className="w-full border-2 border-slate-100 p-3 rounded-xl font-bold text-slate-700 outline-none focus:border-[#1a3826]">
                ...
                <div className="flex gap-4 items-end">
                    <div className="flex-1 space-y-1">
                        ...
                        <input type="number" value={formData.entitlement} onChange={e => setFormData({...formData, entitlement: Number(e.target.value)})} className="w-full border-2 border-slate-100 p-3 rounded-xl font-bold text-slate-700 outline-none focus:border-[#1a3826]"/>
                    </div>
                    <div className="flex-1 space-y-1">
                        <label className="text-[10px] font-bold text-slate-400 uppercase">Preneseno</label>
                        <input type="number" value={formData.carryover} onChange={e => setFormData({...formData, carryover: Number(e.target.value)})} className="w-full border-2 border-slate-100 p-3 rounded-xl font-bold text-slate-700 outline-none focus:border-[#1a3826]"/>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div className="p-8 bg-slate-50/50">
            <h3 className="text-sm font-black text-slate-800 uppercase mb-4 flex gap-2"><CheckSquare className="text-[#FFC72C]"/> Permisije Sistema</h3>
            <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                {AVAILABLE_PERMISSIONS.map((perm) => (
                    <div key={perm.id} onClick={() => handlePermissionChange(perm.id)} 
                         className={`p-3 rounded-xl border-2 cursor-pointer flex items-center gap-2 font-bold text-xs transition-all ${formData.permissions[perm.id] ? 'border-[#1a3826] bg-[#1a3826]/10 text-[#1a3826]' : 'border-slate-200 text-slate-400 bg-white hover:border-slate-300'}`}>
                        <div className={`w-4 h-4 rounded flex items-center justify-center border ${formData.permissions[perm.id] ? 'bg-[#1a3826] border-[#1a3826]' : 'bg-white border-slate-300'}`}>
        ...
        <div className="p-6 bg-white border-t border-slate-100 flex justify-end gap-3">
            {onClose ? (
                <button type="button" onClick={onClose} className="px-6 py-3 rounded-xl font-bold text-slate-500 hover:bg-slate-100">Zatvori</button>
            ) : (
                <button type="button" onClick={() => router.back()} className="px-6 py-3 rounded-xl font-bold text-slate-500 hover:bg-slate-100">Nazad</button>
            )}
