...
export default function SettingsModal({ year, initialGoals, initialScale, restaurants, currentUserId, onClose }: SettingsModalProps) {
  ...
  const addScaleLevel = () => {
    setScale([...scale, { label: "Neue Stufe", min: 0, max: 0, colorHex: "#000000" }]);
  };
        ...
        <div className="bg-white border-b px-6 py-4 flex justify-between items-center shrink-0">
          <div>
            <div className="flex items-center gap-2 mb-1">
                ...
                <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">VORLAGE</span>
            </div>
            <h2 className="text-xl font-bold text-slate-800 tracking-tight">PDS-Konfiguration</h2>
            <p className="text-xs text-slate-500 font-medium uppercase tracking-wider">Jahr: {year} (2025–2030)</p>
          </div>
          ...
          <div className="w-64 bg-slate-50 border-r flex flex-col shrink-0">
            ...
            {activeTab === 'goals' && (
                <div className="flex-1 overflow-y-auto px-4 pb-4">
                    <button onClick={addGoal} className="w-full text-xs bg-slate-200 hover:bg-slate-300 py-2 rounded-lg font-bold text-[#1a3826] mb-3">+ Ziel hinzufügen</button>
                    ...
                    {goals.map((g, i) => (
                        <div key={i} className="group flex items-center gap-2">
                            ...
                            >
                                <span className="truncate">{g.title || `Ziel ${i+1}`}</span>
          ...
          <div className="flex-1 overflow-y-auto bg-white p-8">
            ...
            {activeTab === 'restaurants' && (
              <div className="space-y-6 max-w-2xl mx-auto">
                <p className="text-sm text-slate-600">Wählen Sie die Restaurant(s), auf die die Vorlage angewendet wird.</p>
                <label className="flex items-center gap-3 p-4 rounded-xl border-2 border-slate-200 hover:border-[#1a3826]/50 cursor-pointer transition-colors">
                  ...
                  />
                  <span className="font-bold text-slate-800">Alle Restaurants</span>
                </label>
                {!allRestaurants && (
            ...
            {activeTab === 'goals' && goals[selectedGoalIndex] && (
              <div className="space-y-8 max-w-3xl mx-auto animate-in slide-in-from-right-4 duration-300">
                  ...
                  {/* Naziv */}
                  <div>
                      <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">Zielbezeichnung</label>
                      <input 
                          ...
                          placeholder="z. B. Kaffeeumsatz"
                      />
                  </div>

                  {/* Tip */}
                  <div>
                      <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-3">Punktetyp</label>
                      <div className="flex gap-4">
                          ...
                          >
                              <Hash size={24}/>
                              <span className="text-xs font-bold uppercase">Bereich</span>
                          </button>
                          ...
                          >
                              <ToggleLeft size={24}/>
                              <span className="text-xs font-bold uppercase">Ja / Nein</span>
                          </button>
                      </div>
                  </div>
                  ...
                  {goals[selectedGoalIndex].type === 'NUMERIC' && (
                      <div className="bg-slate-50 rounded-2xl p-6 border border-slate-100">
                          <div className="flex justify-between items-center mb-4">
                              <h4 className="text-xs font-black text-slate-700 uppercase">Punkteregeln</h4>
                              <button onClick={() => addRule(selectedGoalIndex)} className="text-[10px] font-bold text-white bg-blue-500 hover:bg-blue-600 px-3 py-1.5 rounded-lg transition-colors">+ Regel hinzufügen</button>
                          </div>
                          <div className="space-y-2">
                  ...
                  {goals[selectedGoalIndex].type === 'BOOLEAN' && (
                      <div className="bg-slate-50 rounded-2xl p-6 border border-slate-100">
                          <h4 className="text-xs font-black text-slate-700 uppercase mb-4">Punkte-Konfiguration</h4>
                          
                          <div className="flex items-center gap-6 mb-4">
                              <div className="flex-1 bg-white p-4 rounded-xl border border-slate-200 shadow-sm">
                                  <div className="flex items-center justify-between mb-2">
                                      <span className="text-sm font-bold text-slate-700">Antwort &quot;Ja&quot;</span>
                                  ...
                                  <div className="flex items-center gap-2">
                                      ...
                                      />
                                      <span className="text-xs font-bold text-slate-400 uppercase">Punkte</span>
                                  </div>
                              </div>
                              
                              <div className="flex-1 bg-white p-4 rounded-xl border border-slate-200 shadow-sm">
                                  <div className="flex items-center justify-between mb-2">
                                      <span className="text-sm font-bold text-slate-700">Antwort &quot;Nein&quot;</span>
                          ...
                          <p className="text-[10px] text-slate-400 text-center">
                              * Negative Zahl für Punkteabzug (z. B. -5).
                          </p>
                      </div>
                  )}
              </div>
            ...
            {activeTab === 'scale' && (
                <div className="space-y-4 max-w-2xl mx-auto">
                    {scale.map((s, i) => (
                        <div key={i} className="grid grid-cols-[1fr_72px_72px_48px_40px] items-end gap-3 bg-white p-4 rounded-xl border border-slate-200 shadow-sm">
                            <div className="min-w-0"><label className="text-[10px] font-bold text-slate-400 uppercase block mb-1">Bezeichnung</label><input value={s.label} onChange={(e) => {const n=[...scale]; n[i].label=e.target.value; setScale(n)}} className="w-full font-bold text-slate-800 border-b border-transparent focus:border-slate-300 outline-none text-center" style={{color:s.colorHex}}/></div>
                            <div className="text-center"><label className="text-[10px] font-bold text-slate-400 uppercase block mb-1">Min</label><input type="number" value={s.min} onChange={(e) => {const n=[...scale]; n[i].min=parseFloat(e.target.value); setScale(n)}} className="w-full bg-slate-50 rounded p-1 text-center font-bold"/></div>
                            <div className="text-center"><label className="text-[10px] font-bold text-slate-400 uppercase block mb-1">Max</label><input type="number" value={s.max} onChange={(e) => {const n=[...scale]; n[i].max=parseFloat(e.target.value); setScale(n)}} className="w-full bg-slate-50 rounded p-1 text-center font-bold"/></div>
                            <div className="flex flex-col items-center"><label className="text-[10px] font-bold text-slate-400 uppercase block mb-1">Farbe</label><input type="color" value={s.colorHex} onChange={(e) => {const n=[...scale]; n[i].colorHex=e.target.value; setScale(n)}} className="h-8 w-8 cursor-pointer border-0 rounded"/></div>
                            <button type="button" onClick={() => {const n=[...scale]; n.splice(i,1); setScale(n)}} className="text-slate-300 hover:text-red-500 self-center pb-1"><Trash2 size={18}/></button>
                        </div>
                    ))}
                    <button onClick={addScaleLevel} className="w-full py-3 bg-slate-50 border border-dashed border-slate-300 rounded-xl text-xs font-bold text-slate-500 hover:bg-slate-100">+ Stufe hinzufügen</button>
                </div>
            )}
          </div>
        </div>
