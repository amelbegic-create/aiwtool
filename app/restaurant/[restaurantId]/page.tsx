...
import { 
  Store, MapPin, Calculator, ClipboardCheck, 
...
export default async function RestaurantDashboard({ params }: Props) {
  ...
  const restaurant = await prisma.restaurant.findUnique({
    where: { code: params.restaurantId }
  });
  ...
  const isSuperAdmin = userRole === 'SUPER_ADMIN' || userRole === 'DIRECTOR' || userRole === 'ADMIN';
  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC] p-6 md:p-8 text-[#0F172A]">
      <div className="max-w-6xl mx-auto">
        ...
        <Link href="/restaurants" className="inline-flex items-center gap-2 text-slate-500 text-sm font-bold hover:text-[#1a3826] mb-6 transition-colors">
            ...
            <div className="flex gap-6 items-center">
                ...
                <div>
                    <div className="flex items-center gap-3 mb-1">
                        ...
                        <span className={`px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-wider ${restaurant.isActive ? 'bg-emerald-100 text-emerald-700' : 'bg-red-100 text-red-700'}`}>
                    ...
                    <div className="flex items-center gap-4 text-sm text-slate-500">
                        ...
                        <span className="flex items-center gap-1"><MapPin className="w-4 h-4" /> {restaurant.address || "BiH"}</span>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                {/* 1. MJESEČNI PLANER (BLUE) */}
                <Link href={`/tools/labor-planner?restaurant=${restaurant.code}`} className="group bg-white p-6 rounded-xl border border-slate-200 hover:border-blue-500 hover:shadow-md transition-all flex flex-col h-60 relative overflow-hidden">
                ...
                {/* 2. EVALUACIJA UČINKA (PURPLE) - SADA AKTIVNO! */}
                <Link href={`/tools/evaluations?restaurant=${restaurant.code}`} className="group bg-white p-6 rounded-xl border border-slate-200 hover:border-purple-500 hover:shadow-md transition-all flex flex-col h-60 relative overflow-hidden">
                ...
                {/* 3. PLAN PRODUKTIVNOSTI (ORANGE) - SADA AKTIVNO! */}
                <Link href={`/tools/productivity?restaurant=${restaurant.code}`} className="group bg-white p-6 rounded-xl border border-slate-200 hover:border-orange-500 hover:shadow-md transition-all flex flex-col h-60 relative overflow-hidden">
